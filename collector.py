...
class OutputWriters:
    ...
    def write(self, rec: Dict):
        ...
        if self.kml_file and rec.get("lat") is not None and rec.get("lon") is not None:
            ...
            safe_name = html.escape(str(name)) if name is not None else ""
            # Sanitize CDATA terminator so embedded "]]>" cannot escape the section.
            desc = json.dumps(rec).replace("]]>", "]]]]><![CDATA[>")
            self.kml_file.write(
                f'<Placemark><name>{safe_name}</name><description><![CDATA[{desc}]]></description>'
...
class NmeaBroadcaster:
    ...
    async def start(self):
        # Bind to loopback only by default. NMEA contains live GPS coordinates;
        # exposing it on every interface (host="0.0.0.0") would broadcast the
        # operator's location to the LAN. If LAN access is genuinely needed,
        # add an explicit --bind flag and document the privacy trade-off.
        self.server = await asyncio.start_server(self._handle_client, host="127.0.0.1", port=self.port)
        print(f"NMEA TCP server listening on 127.0.0.1:{self.port}")
...
async def handle_device(serial: str, outputs: OutputWriters, gps_only: bool = False, nmea: Optional[NmeaBroadcaster] = None):
    # Set up port forwarding so host can reach device abstract sockets.
    print(f"[{serial}] setting adb forward tcp:{PORT} -> localabstract:cellstream_data")
    subprocess.run(["adb", "-s", serial, "forward", f"tcp:{PORT}", "localabstract:cellstream_data"], check=True)
    try:
        print(f"[{serial}] setting adb forward tcp:{NMEA_PHONE_PORT} -> localabstract:cellstream_nmea")
        subprocess.run(
            ["adb", "-s", serial, "forward", f"tcp:{NMEA_PHONE_PORT}", "localabstract:cellstream_nmea"],
            check=True,
        )
    ...
    try:
        subprocess.run(
            [
                ...
                "-n",
                "dev.alsatianconsulting.cellulardatasource/.CellStreamService",
            ],
