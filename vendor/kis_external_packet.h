...
/* Minimal v3 sub-block helpers (not present in upstream header without the full tree) */
typedef struct {
    uint16_t type;
    uint16_t pad0;
    uint8_t data[0];
} __attribute__((packed)) kismet_v3_sub_block;

typedef struct {
    uint16_t length;
    uint16_t pad0;
    uint8_t data[0];
} __attribute__((packed)) kismet_v3_sub_string;

static inline size_t ks_proto_v3_pad(size_t len) {
    size_t rem = len % 4;
    return rem ? (4 - rem) : 0;
}
