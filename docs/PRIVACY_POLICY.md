...
On Android, local stream endpoints are still reachable by other apps on the same phone when those apps can open localhost or abstract-socket connections. Do not enable streaming on a phone that also runs untrusted apps.
...
- Use trusted phones. Other local apps may be able to connect to enabled phone-side stream endpoints and read cellular or GPS telemetry while streaming is active.
