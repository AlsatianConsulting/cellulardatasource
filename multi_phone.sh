...
# Raspberry Pi OS / Debian adb can miss newer Pixel devices with the default
# backend. Prefer libusb unless the installer/operator explicitly overrides it.
...
if [[ ${FORWARD_GPS} -eq 1 ]]; then
  echo "[*] GPS forwarding enabled on host port ${GPS_PORT} (device localabstract:cellstream_nmea, first device only)"
fi
...
for serial in "${SERIALS[@]}"; do
  ...
  if ! "${ADB_BIN}" -s "${serial}" forward "tcp:${port}" "localabstract:cellstream_data"; then
    echo "[!] Failed adb forward for ${serial} tcp:${port}->localabstract:cellstream_data; skipping device" >&2
    continue
  fi
  ...
  if [[ ${FORWARD_GPS} -eq 1 && ${idx} -eq 0 ]]; then
    ...
    "${ADB_BIN}" -s "${serial}" forward "tcp:${GPS_PORT}" "localabstract:cellstream_nmea" || true
  fi
  idx=$((idx + 1))
done
