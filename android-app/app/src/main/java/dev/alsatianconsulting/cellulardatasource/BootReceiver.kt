...
class BootReceiver : BroadcastReceiver() {
    ...
    private fun shouldStartOnBoot(context: Context): Boolean {
        ...
        val unlocked = try {
            ...
            Log.w(TAG, "Unable to determine user unlock state for service startup; assuming unlocked", exc)
            true
        }
    ...
    private fun shouldLaunchUiOnBoot(context: Context, action: String): Boolean {
        ...
        val unlocked = try {
            ...
            Log.w(TAG, "Unable to determine user unlock state for UI launch; assuming unlocked", exc)
            true
        }
        if (!unlocked) return false
