...
public:
    kis_datasource_cell(shared_datasource_builder in_builder) :
        ...
        set_int_source_cap_interface("cell");
...
public:
    ...
    static int PacketHandler(CHAINCALL_PARMS) {
        ...
        auto hash_mac = [](const std::string& seed, uint8_t prefix) -> mac_addr {
        ...
        mac_addr mac = hash_mac(fullid, 0x02);
        ...
        common->dest = Globalreg::globalreg->broadcast_mac;
...
public:
    ...
    virtual void initialize() override {
        ...
        // Only allow explicit type=cell source definitions.
        // Probe mode can incorrectly claim unrelated interfaces on some Kismet builds.
...
extern "C" {
    ...
    int kis_plugin_activate(global_registry *in_globalreg) {
        try {
            _MSG("cell plugin loaded (PHY+UI only; remote helper mode)", MSGFLAG_INFO);

            // Register the datasource builder so type=cell sources can launch
            auto datasourcetracker = Globalreg::fetch_mandatory_global_as<datasource_tracker>();
            datasourcetracker->register_datasource(
                std::make_shared<datasource_cell_builder>());
