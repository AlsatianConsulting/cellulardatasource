...
# Validates v1 source-line format: tcp://HOST:PORT[:key=value][,key=value...].
# Update this pattern when a new format version is introduced.
_DEFINITION_RE = re.compile(
    r"^tcp://[a-zA-Z0-9._-]+:\d{1,5}"
    r"(?::[a-zA-Z0-9._-]+=[^,\s]+)?"
    r"(?:,[a-zA-Z0-9._-]+=[^,\s]+)*$"
)


def _is_valid_definition(definition: str) -> bool:
    return bool(_DEFINITION_RE.match(definition))
    ...
    with open(path, "r", encoding="utf-8", errors="replace") as infile:
        for raw in infile:
            ...
            if not _is_valid_definition(definition):
                log(f"skipping malformed definition: {definition!r}")
                continue
